...
This component is based on the `span` element and supports [common margin props](/research/docs/overview/layout#margin-props).
...
The default trim values are configured for the system font stack that’s used by Radix Themes. If you are using custom fonts, you can [adjust](/research/docs/theme/typography#customization) the trim values using the corresponding CSS variables.
...
Use the `color` prop to assign a specific [color](/research/docs/theme/color). The text colors are designed to achieve at least Lc 60 APCA contrast over common background colors.
...
Composing `Text` with a form control like [`Checkbox`](/research/docs/components/checkbox), [`RadioGroup`](/research/docs/components/radio-group), or [`Switch`](/research/docs/components/switch) automatically centers the control with the first line of text, even when the text is multi-line.
