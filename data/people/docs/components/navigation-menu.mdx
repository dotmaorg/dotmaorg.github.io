...
requires an adjusted DOM structure or if you need flexibility to achieve [advanced animation](/peoples/docs/components/navigation-menu#advanced-animation).
...
Submenus work differently to `Root` navigation menus and are similar to [`Tabs`](/peoples/docs/components/tabs) in that one item should always be active, so be
...
See [this example](/peoples/docs/components/navigation-menu#with-client-side-routing) for more information on usage with third party routing components.
