...
The resulting `border-radius` is contextual and differs depending on the component. For example, when set to `full`, a [Button](/research/docs/components/button) becomes pill-shaped, while a [Checkbox](/research/docs/components/checkbox) will never become fully rounded to prevent any confusion between it and a [Radio](/research/docs/components/radio).
