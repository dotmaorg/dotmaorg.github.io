				...
				>
					...
					{hasPlaygroundExample(frontmatter.slug) && (
						<NextLink
							passHref
							legacyBehavior
							href={`/research/playground#${frontmatter.slug}`}
						>
...
export async function getStaticPaths() {
	const frontmatters = getAllFrontmatter("products/docs/components");

	return {
		paths: frontmatters.map((frontmatter) => ({
			params: { slug: frontmatter.slug.replace("products/docs/components/", "") },
		})),
		fallback: false,
	};
}
