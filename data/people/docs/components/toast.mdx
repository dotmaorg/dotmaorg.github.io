...
The toast that automatically closes. It should not be held open to [acquire a user response](/peoples/docs/components/toast#action).
		...
		},
		{
			...
			description: (
				<span>
					Control the{" "}
					<Link
						href="/peoples/docs/components/toast#sensitivity"
						variant="blue"
					>
						sensitivity
					</Link>{" "}
...
When obtaining a user response is necessary, portal an [`AlertDialog`](/peoples/docs/components/alert-dialog) styled as a toast into the viewport instead.
...
When a toast must appear every time a user clicks a button, use state to render multiple instances of the same toast (see below). Alternatively, you can abstract the parts to create your own [imperative API](/peoples/docs/components/toast#imperative-api).
...
Create your own imperative API to allow [toast duplication](/peoples/docs/components/toast#duplicate-toasts) if preferred.
